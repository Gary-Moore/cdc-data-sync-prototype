...
# Deploy CdcReceiver as an Azure Container App
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="$(dirname "$SCRIPT_DIR")"
REPO_ROOT="$(dirname "$PROJECT_DIR")" 
DOCKERFILE_PATH="$REPO_ROOT/receiver.dockerfile"
IMAGE_NAME="cdc-receiver"
...
: "${CONTAINER_APP_NAME:=cdc-receiver-app}"
: "${CONTAINER_REGISTRY_NAME:=cdcreceiveracr}"
: "${LOCATION:=uksouth}"
...
az containerapp create \
  ...
  --env-vars \
    ConnectionStrings__ReceiverDb="$SQL_CONNECTION_STRING" \
    Receiver__ServiceBusConnectionString="$SERVICE_BUS_CONNECTION_STRING" \
    Receiver__TopicName="$SERVICE_BUS_TOPIC_NAME" \
    Receiver__SubscriptionName="$SERVICE_BUS_SUBSCRIPTION_NAME"
